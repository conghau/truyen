...
<div class="static_all_wrap">
	...
	{else}
		...
		{$fixed_base_url|cat:'user/store'|form_open:'class=form-horizontal id=form-main method=POST'}
			<table class="mem_input">
				...
				<tr>
					<input type="hidden" name="auth_method" value="{$user_info['auth_method']|h}">
					{if $user_info['auth_method'] eq $list_auth_method['auth_image']['id']}
						<th><label>{'label_user_auth_by_image'|lang}</th>
						<td><label>{'label_attachments'|lang}</label></td>
					{else}
						<th><label>{'label_user_auth_by_info'|lang}</label></th>
						<td>
							<label>{'label_user_confirm_organization'|lang}：{$user_info['confirm_organization']}</label><input type="hidden" name="confirm_organization" value="{$user_info['confirm_organization']|h}"><br><br>
							<label>{'label_user_phone_number'|lang}：{$user_info['confirm_phone_number']}</label><input type="hidden" name="confirm_phone_number" value="{$user_info['confirm_phone_number']|h}"><br /> 
						</td>
					{/if}
				</tr>
				<!-- language -->
		...
		<p class="mt20">{'label_user_regist_confirm_introduce_bottom'|lang}</p>	
			<div class="mt20 ct">
				...
				<input type="image" src="{$fixed_base_url}assets/img/{$language}/btn_shinsei.png" id="btn_create" value="{'label_button_member_regist'|lang}" style="border:none">
			</div>
		</div>
				...
				<div class="modal">
					<div class="ctbox">
						<div class="ctinner_m">
							...
							<p class="ct m20">
								<a id="btn_yes" class="modalClose" href="{$fixed_base_url}login"><img src="{$fixed_base_url}assets/img/{$language}/modal_hai.png"></a>
							</p>
						</div><!--/ctinner-->
					</div><!--/ctbox-->
				</div>
	...
	<script language="Javascript">
		$(document).ready(function(){
			$("#btn_back").on('click', function() {
				$('#form-main').attr('action', '{/literal}{$fixed_base_url|cat:'user/create'}{literal}');
				$('#form-main').submit();
			});
